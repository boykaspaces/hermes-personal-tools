...
func (h *LambdaHTTPHandler) Handle(ctx context.Context, event APIGatewayV2Request) (APIGatewayV2Response, error) {
	...
	request, err := http.NewRequestWithContext(ctx, event.RequestContext.HTTP.Method, requestURL.String(), body)
	if err != nil {
		return APIGatewayV2Response{}, fmt.Errorf("construct HTTP request: %w", err)
	}
	for name, value := range event.Headers {
		// API Gateway removes the dedicated client-token header before the
		// integration. Drop it and the standard Authorization header again as
		// defense in depth so neither reaches the MCP/application layer.
		if strings.EqualFold(name, ClientTokenHeader) || strings.EqualFold(name, "Authorization") {
			continue
		}
	...
	callerID, _ := event.RequestContext.Authorizer.Lambda["caller_id"].(string)
	request = request.WithContext(identity.WithCaller(request.Context(), identity.New(callerID)))
